...
def test_add_turma_com_professor_existente(client):
    # Cria o professor
    response_professor = client.post('/professores', json={"nome": "Prof. Carlos"})
    professor_id = response_professor.get_json()['id']
    res = client.post('/turmas', json={"nome": "Turma A", "professor_id": professor_id})
    assert res.status_code == 201
...
def test_update_turma(client):
    client.post('/turmas', json={"nome": "Turma C", "professor_id": 1})
    res = client.put('/turmas/2', json={"nome": "Turma C+", "professor_id": 1})
    assert res.status_code == 200
...
def test_add_aluno(client):
    client.post('/turmas', json={"nome": "Turma D", "professor_id": 5})
    res = client.post('/alunos', json={"nome": "Maria", "turma_id": 3})
    assert res.status_code == 201
...
def test_update_aluno(client):
    client.post('/alunos', json={"nome": "João", "turma_id": 3})
    res = client.put('/alunos/2', json={"nome": "João Pedro", "turma_id": 3})
    assert res.status_code == 200
